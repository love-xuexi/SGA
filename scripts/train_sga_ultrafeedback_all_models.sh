...
# Usage: time bash ../scripts/train_sga_ultrafeedback_all_models.sh qwen4b
# This script trains SGA (SGA) on UltraFeedback dataset with multiple models
...
declare -A MODEL_CONFIGS=(
    [llama1b]="llama1b|2,3,4,5,6,7|6|9956|path/to/models/Llama-3.2-1B-Instruct|path/to/SGA/outputs/ultrafeedback/llama-1b|path/to/SGA/logs/ultrafeedback/llama-1b|2|8|False"
    [llama8b]="llama8b|2,3,4,5,6,7|6|9928|path/to/models/Llama-3.1-8B-Instruct|path/to/SGA/outputs/ultrafeedback/llama-8b|path/to/SGA/logs/ultrafeedback/llama-8b|1|16|True"
    [qwen7b]="qwen7b|2,3,4,5,6,7|6|9988|path/to/models/qwen-2.5-7b-instruct|path/to/SGA/outputs/ultrafeedback/qwen-7b|path/to/SGA/logs/ultrafeedback/qwen-7b|1|16|True"
    [llama3b]="llama3b|2,3,4,5,6,7|6|4127|path/to/models/Llama-3.2-3B-Instruct|path/to/SGA/outputs/ultrafeedback/llama-3b|path/to/SGA/logs/ultrafeedback/llama-3b|1|16|False"
    [qwen8b]="qwen8b|2,3,4,5,6,7|6|7218|path/to/models/Qwen3-8B|path/to/SGA/outputs/ultrafeedback/qwen-8b|path/to/SGA/logs/ultrafeedback/qwen-8b|1|16|True"
    [qwen4b]="qwen4b|2,3,4,5,6,7|6|9688|path/to/models/Qwen3-4B|path/to/SGA/outputs/ultrafeedback/qwen-4b|path/to/SGA/logs/ultrafeedback/qwen-4b|1|16|True"
)

# Dataset configuration
DATASET="path/to/exp/datasets/ultrafeedback-binarized-preferences-cleaned-kto"
report_to="wandb"
max_length=1024
num_train_epochs=2
learning_rate=1e-5
wandb_name_prefix="sga_ultrafeedback"
    ...
    do
        if [ "$mode" == "dpo_bt" ]; then
            ...
            do
                ...
                echo "Starting UltraFeedback SGA Training Task: [$model_name]"
...
main() {
    if [ "$TARGET_MODEL" == "all" ]; then
        ...
        echo ""
        echo "✓ All UltraFeedback SGA models have been trained successfully!"
    else
...
if [ "$TARGET_MODEL" == "-h" ] || [ "$TARGET_MODEL" == "--help" ]; then
    echo "Usage: bash train_sga_ultrafeedback_all_models.sh [model_name]"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  bash train_sga_ultrafeedback_all_models.sh llama1b      # Train only Llama 1B"
    echo "  bash train_sga_ultrafeedback_all_models.sh all          # Train all models"
    echo "  bash train_sga_ultrafeedback_all_models.sh              # Train all models (default)"
    exit 0
fi
