...
# time bash ../scripts/train_sga_hh_rlhf_all_models.sh
# time bash ../scripts/train_sga_ultrafeedback_all_models.sh
# time bash ../scripts/train_sga_helpsteer2_all_models.sh
# Usage: time bash ../scripts/train_sga_hh_rlhf_all_models.sh llama1b
# This script trains SGA (SGA) on HH-RLHF dataset with multiple models
...
declare -A MODEL_CONFIGS=(
    [llama1b]="llama1b|2,3,4,5,6,7|6|9956|path/to/models/Llama-3.2-1B-Instruct|path/to/SGA/outputs/hh-rlhf/llama-1b|path/to/SGA/logs/hh-rlhf/llama-1b|2|8|False"
    [llama8b]="llama8b|2,3,4,5,6,7|6|9928|path/to/models/Llama-3.1-8B-Instruct|path/to/SGA/outputs/hh-rlhf/llama-8b|path/to/SGA/logs/hh-rlhf/llama-8b|1|16|True"
    [qwen7b]="qwen7b|2,3,4,5,6,7|6|9988|path/to/models/qwen-2.5-7b-instruct|path/to/SGA/outputs/hh-rlhf/qwen-7b|path/to/SGA/logs/hh-rlhf/qwen-7b|1|16|True"
    [llama3b]="llama3b|2,3,4,5,6,7|6|4127|path/to/models/Llama-3.2-3B-Instruct|path/to/SGA/outputs/hh-rlhf/llama-3b|path/to/SGA/logs/hh-rlhf/llama-3b|1|16|False"
    [qwen8b]="qwen8b|2,3,4,5,6,7|6|7218|path/to/models/Qwen3-8B|path/to/SGA/outputs/hh-rlhf/qwen-8b|path/to/SGA/logs/hh-rlhf/qwen-8b|2|8|True"
    [qwen4b]="qwen4b|2,3,4,5,6,7|6|9688|path/to/models/Qwen3-4B|path/to/SGA/outputs/hh-rlhf/qwen-4b|path/to/SGA/logs/hh-rlhf/qwen-4b|1|16|False"
)

# Dataset configuration
DATASET="path/to/user/train_harmhelp_llama3"
report_to="wandb"
max_length=1024
num_train_epochs=2
learning_rate=1e-5
wandb_name_prefix="sga_hh_rlhf"
    ...
    do
        if [ "$mode" == "dpo_bt" ]; then
            ...
            do
                ...
                echo "Starting HH-RLHF SGA Training Task: [$model_name]"
...
main() {
    if [ "$TARGET_MODEL" == "all" ]; then
        ...
        echo ""
        echo "✓ All HH-RLHF SGA models have been trained successfully!"
    else
...
if [ "$TARGET_MODEL" == "-h" ] || [ "$TARGET_MODEL" == "--help" ]; then
    echo "Usage: bash train_sga_hh_rlhf_all_models.sh [model_name]"
    echo ""
    echo "Supported models:"
    echo "  gemma2b   - Google Gemma 2B Instruct"
    ...
    echo ""
    echo "Examples:"
    echo "  bash train_sga_hh_rlhf_all_models.sh gemma2b      # Train only Gemma 2B"
    echo "  bash train_sga_hh_rlhf_all_models.sh all          # Train all models"
    echo "  bash train_sga_hh_rlhf_all_models.sh              # Train all models (default)"
    exit 0
fi
