...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Get the hidden state at the last token position for each sample
        batch_indices = torch.arange(len(last_hidden_state), device=last_hidden_state.device)
        last_hidden = last_hidden_state[batch_indices, last_index]
        value = self.v_head(last_hidden)
        # Handle both single-dim and multi-dim outputs
    ...
    def forward_regression_only(
        ...
        **kwargs,
        ...
        """
        Forward pass for regression head only (used for MSE loss calculation).
        
        Args:
            input_ids: Input token ids (original prompt without score info)
            attention_mask: Attention mask
            apply_sigmoid: Whether to apply sigmoid to normalize output to [0,1]
            
        Returns:
            Predicted scores with shape (batch_size, num_output), normalized to [0,1] if apply_sigmoid=True
        """
        ...
        # Get the last token position for each sample
        last_index = attention_mask.sum(dim=-1) - 1
        batch_indices = torch.arange(len(last_hidden_state), device=last_hidden_state.device)
        last_hidden = last_hidden_state[batch_indices, last_index]
        
        # Pass through value head
        predicted_scores = self.v_head(last_hidden)
        ...
        if predicted_scores.dtype != torch.float32:
