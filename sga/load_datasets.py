...
def build_dataset_HS2(data_path, tokenizer, split='train', size=None, mode='', model_name=''):
    ...
    try:
        ...
        ds = load_dataset("/mnt/ai4s/lzy_exp/datasets/HelpSteer2", split=split)
    ...
    print(f"Rows after building paired data: {len(ds)}")
...
def build_dataset_UF(data_path, tokenizer, split='train', size=None, mode='', model_name=''):
    ...
    try:
        ...
        ds = load_dataset("/mnt/ai4s/lzy_exp/datasets/ultrafeedback-binarized-preferences-cleaned-kto", split=split)
...
def build_dataset_HH(data_path, tokenizer, split='train', size=None, mode='', model_name=''):
    ...
    try:
        ...
        ds = load_from_disk('/mnt/ai4s/zhouhaojie/liuzhanyang/21/RiC/ric/datasets/train_harmhelp_llama3')
    ...
    for prompt, group in grouped:
        ...
        if int(row_a['label']) == 1 and int(row_b['label']) == 0:
            ...
            continue # Skip due to label conflict

        # Calculate margin (Margin usually keeps the original score difference for Loss calculation, no normalization needed, or depending on your Loss needs)
