...
def load_main_tokenizer(tokenier_name):
    """
    Load tokenizer with proper special tokens for different model families.
    Supports: LLaMA 2, LLaMA 3.x, Qwen 2.5, Qwen 3, etc.
    """
    tokenizer = AutoTokenizer.from_pretrained(tokenier_name, use_fast=False)
    
    # Only add missing special tokens, don't override existing ones
    special_tokens_to_add = {}
    
    # Handle pad token - most important for training
    ...
    # Handle other special tokens only if missing
...
# Common special tokens for different model families
SPECIAL_TOKENS_TO_STRIP = [
    # LLaMA 2
    '[PAD]', '<s>', '</s>', '<unk>',
    # LLaMA 3.x
    '<|begin_of_text|>', '<|end_of_text|>', '<|eot_id|>', 
    '<|start_header_id|>', '<|end_header_id|>', '<|pad|>',
    # Qwen 2.5 / Qwen 3
    '<|im_start|>', '<|im_end|>', '<|endoftext|>',
    # Other common tokens
    '<pad>', '</s>', '<start_header_id>', '<end_header_id>',
]


def _strip_special_tokens(text):
    """Strip all known special tokens from text."""
...
def get_clean_data(full_responses, full_prompts, remove_bad=False):
    """
    Clean generated responses by removing special tokens and unwanted patterns.
    Supports multiple model families: LLaMA 2/3.x, Qwen 2.5/3, etc.
    """
    full_prompts_clean = []
    full_responses_clean = []
    for i, response in enumerate(full_responses):
        # Strip all known special tokens
        full_prompts[i] = _strip_special_tokens(full_prompts[i]).rstrip()
        response = _strip_special_tokens(response)
        temp_resp = response.replace(full_prompts[i], '').strip()
        # Remove trailing content after EOS-like tokens
        for eos_token in ['</s>', '<|end_of_text|>', '<|eot_id|>', '<|im_end|>', '<|endoftext|>']:
            if eos_token in temp_resp:
                temp_resp = temp_resp[:temp_resp.rindex(eos_token)]
        # Remove content after BOS-like tokens
        for bos_token in ['<s>', '<|begin_of_text|>']:
            if bos_token in temp_resp:
                temp_resp = temp_resp[temp_resp.rindex(bos_token):]
        # Remove continued conversation patterns
