...
class ValueHead(nn.Module):
    ...
    def get_output_dim(self):
        """Return the output dimension of the value head"""
        if self.layer_type == 'linear':
            return self.summary.out_features
        else:
            return self.summary[-1].out_features
...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # use the last token value as reward/preference weights
        ...
        # If output dimension is 1, squeeze to maintain backward compatibility
        if self.v_head.get_output_dim() == 1:
        ...
        return (lm_logits, loss, value)  # Return value logits
...
def load_model_withhead(model_name, peft_name, tokenizer, device, \
        layer_type='linear', num_neurons=1024, num_layers=1, num_output=1, load_in_8bit=False):
...
def model_withhead_forward(model, input_ids, attention_mask, device, forward_type='reward', labels=None):
    ...
    Args:
        forward_type: 'reward' for preference scoring, 'dpo' for calculating logps.
    """
    if forward_type == 'reward':
        _, _, reward_logits = model(input_ids.to(device), attention_mask=attention_mask.to(device))
        
        preference_activation = getattr(model.config, 'preference_activation', 'sigmoid')
        
        if preference_activation == 'sigmoid':
            return torch.sigmoid(reward_logits)
        elif preference_activation == 'softmax':
            return torch.nn.functional.softmax(reward_logits, dim=-1)
        elif preference_activation == 'none':
            return reward_logits
        else:
            print(f"⚠️  Warning: Unknown preference_activation '{preference_activation}', using 'sigmoid' as default.")
            return torch.sigmoid(reward_logits)
        ...
        raise NotImplementedError(f"Unknown forward_type: {forward_type}")